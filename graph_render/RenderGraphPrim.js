...
function RenderGraphPrim(props) {
...
function handleAddEdge(index) {
  ...
  function renderDirectedEdge() {
    const edgesComponents = multiEdges.flatMap((edge, index) => {
        ...
        let x1 = edge.beginVertex.x, x2 = edge.endVertex.x;
        let y1 = edge.beginVertex.y, y2 = edge.endVertex.y;
        let edgeColor = edge.isInSpanningTree ? theme['stroke'] : theme['stroke'].replace('ff', '80'); // Đổi màu cho cạnh không thuộc cây khung
        lines.push(
          <Line
            key={index + "edge"}
            x1={x1}
...
export default RenderGraphPrim