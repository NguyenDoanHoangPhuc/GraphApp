...
//Ham chuyen qua man hinh tuong ung voi thuat toan

const handleNavigateGraphSearch = (algorithm) => {
  ...
  switch (algorithm) {
    ...
    case 'DFS-Recursion':
    case 'Topo':
      screenName = 'GraphSearch';
      break;
    case 'Prim':
      screenName = 'GraphSearchPrim';
      break;
    default:
      console.error(`Không tìm thấy màn hình tương ứng cho thuật toán ${algorithm}`);
      return;
  }
        ...
        {algorithmList.map((algorithm, index) => {
          return (
            <AnimatedPressable
              ...
              //  handleNavigateGraphSearch(algorithm);
              // }
                if (!needVertex(algorithName[algorithm])) {
                  //here
                  handleNavigateGraphSearch(algorithName[algorithm]);
                }
                else setModalVisible(true);
    
              }}
            >
              <Text style={styles.squareButtonTitle}>{algorithTitle[algorithm]}</Text>
